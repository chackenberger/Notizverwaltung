...
abstract class Person implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildProjekt[] Cross Collection to store aggregation of ChildProjekt objects.
     */
    protected $collProjekts;

    /**
     * @var bool
     */
    protected $collProjektsPartial;
     ...
     * @var ObjectCollection|ChildProjekt[]
     */
    protected $projektsScheduledForDeletion = null;
    ...
    public function setDesc($v)
    {
        ...
        if ($this->desc !== $v) {
            $this->desc = $v;
            $this->modifiedColumns[PersonTableMap::COL_DESC] = true;
        }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->projektsScheduledForDeletion !== null) {
                if (!$this->projektsScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    foreach ($this->projektsScheduledForDeletion as $entry) {
                        $entryPk = [];
                    ...
                    $this->projektsScheduledForDeletion = null;
                }

            }

            if ($this->collProjekts) {
                foreach ($this->collProjekts as $projekt) {
                    if (!$projekt->isDeleted() && ($projekt->isNew() || $projekt->isModified())) {
                        $projekt->save($con);
                    }
                }
            }
            ...
            if ($this->notizsScheduledForDeletion !== null) {
                if (!$this->notizsScheduledForDeletion->isEmpty()) {
                    \NotizQuery::create()
                        ->filterByPrimaryKeys($this->notizsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
    ...
    public function removeNotiz(ChildNotiz $notiz)
    {
        if ($this->getNotizs()->contains($notiz)) {
            ...
            $this->notizsScheduledForDeletion[]= clone $notiz;
            $notiz->setBesitzer(null);
        }
     ...
     * @return ObjectCollection|ChildNotiz[] List of ChildNotiz objects
     */
    public function getNotizsJoinProjekt(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildNotizQuery::create(null, $criteria);
    ...
    public function setPersonProjekts(Collection $personProjekts, ConnectionInterface $con = null)
    {
        ...
        foreach ($personProjektsToDelete as $personProjektRemoved) {
            $personProjektRemoved->setPerson(null);
        }
     ...
     * @return ObjectCollection|ChildPersonProjekt[] List of ChildPersonProjekt objects
     */
    public function getPersonProjektsJoinProjekt(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPersonProjektQuery::create(null, $criteria);
        $query->joinWith('Projekt', $joinBehavior);
    ...
    /**
     * Clears out the collProjekts collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addProjekts()
     */
    public function clearProjekts()
    {
        $this->collProjekts = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Initializes the collProjekts crossRef collection.
     *
     * By default this just sets the collProjekts collection to an empty collection (like clearProjekts());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
     */
    public function initProjekts()
    {
        $this->collProjekts = new ObjectCollection();
        $this->collProjektsPartial = true;

        $this->collProjekts->setModel('\Projekt');
    }

    /**
     * Checks if the collProjekts collection is loaded.
     *
     * @return bool
     */
    public function isProjektsLoaded()
    {
        return null !== $this->collProjekts;
    }

    /**
     * Gets a collection of ChildProjekt objects related by a many-to-many relationship
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return ObjectCollection|ChildProjekt[] List of ChildProjekt objects
     */
    public function getProjekts(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collProjektsPartial && !$this->isNew();
        if (null === $this->collProjekts || null !== $criteria || $partial) {
            if ($this->isNew()) {
                // return empty collection
                if (null === $this->collProjekts) {
                    $this->initProjekts();
                }
            } else {

                $query = ChildProjektQuery::create(null, $criteria)
                    ->filterByPerson($this);
                $collProjekts = $query->find($con);
                if (null !== $criteria) {
                    return $collProjekts;
                }

                if ($partial && $this->collProjekts) {
                    //make sure that already added objects gets added to the list of the database.
                    foreach ($this->collProjekts as $obj) {
                        if (!$collProjekts->contains($obj)) {
                            $collProjekts[] = $obj;
                        }
                    }
                }

                $this->collProjekts = $collProjekts;
                $this->collProjektsPartial = false;
            }
        }

        return $this->collProjekts;
    }

    /**
     * Sets a collection of Projekt objects related by a many-to-many relationship
     * to the current object by way of the person_projekt cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param  Collection $projekts A Propel collection.
     * @param  ConnectionInterface $con Optional connection object
     * @return $this|ChildPerson The current object (for fluent API support)
     */
    public function setProjekts(Collection $projekts, ConnectionInterface $con = null)
    {
        $this->clearProjekts();
        $currentProjekts = $this->getProjekts();

        $projektsScheduledForDeletion = $currentProjekts->diff($projekts);

        foreach ($projektsScheduledForDeletion as $toDelete) {
            $this->removeProjekt($toDelete);
        }

        foreach ($projekts as $projekt) {
            if (!$currentProjekts->contains($projekt)) {
                $this->doAddProjekt($projekt);
            }
        }

        $this->collProjektsPartial = false;
        $this->collProjekts = $projekts;

        return $this;
    }

    /**
     * Gets the number of Projekt objects related by a many-to-many relationship
     * to the current object by way of the person_projekt cross-reference table.
     *
     * @param      Criteria $criteria Optional query object to filter the query
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return int the number of related Projekt objects
     */
    public function countProjekts(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collProjektsPartial && !$this->isNew();
        if (null === $this->collProjekts || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collProjekts) {
                return 0;
            } else {

                if ($partial && !$criteria) {
                    return count($this->getProjekts());
                }
            ...
            return count($this->collProjekts);
        }
    }

    /**
     * Associate a ChildProjekt to this object
     * through the person_projekt cross reference table.
     *
     * @param ChildProjekt $projekt
     * @return ChildPerson The current object (for fluent API support)
     */
    public function addProjekt(ChildProjekt $projekt)
    {
        if ($this->collProjekts === null) {
            $this->initProjekts();
        }

        if (!$this->getProjekts()->contains($projekt)) {
            // only add it if the **same** object is not already associated
            $this->collProjekts->push($projekt);
            $this->doAddProjekt($projekt);
        }
     ...
     * @param ChildProjekt $projekt
     */
    protected function doAddProjekt(ChildProjekt $projekt)
    {
        $personProjekt = new ChildPersonProjekt();

        $personProjekt->setProjekt($projekt);
        ...
        if (!$projekt->isPeopleLoaded()) {
            $projekt->initPeople();
            $projekt->getPeople()->push($this);
        } elseif (!$projekt->getPeople()->contains($this)) {
            $projekt->getPeople()->push($this);
        }
    ...
    /**
     * Remove projekt of this object
     * through the person_projekt cross reference table.
     *
     * @param ChildProjekt $projekt
     * @return ChildPerson The current object (for fluent API support)
     */
    public function removeProjekt(ChildProjekt $projekt)
    {
        if ($this->getProjekts()->contains($projekt)) { $personProjekt = new ChildPersonProjekt();

            $personProjekt->setProjekt($projekt);
            if ($projekt->isPeopleLoaded()) {
                //remove the back reference if available
                $projekt->getPeople()->removeObject($this);
            }
            ...
            $this->collProjekts->remove($this->collProjekts->search($projekt));

            if (null === $this->projektsScheduledForDeletion) {
                $this->projektsScheduledForDeletion = clone $this->collProjekts;
                $this->projektsScheduledForDeletion->clear();
            }

            $this->projektsScheduledForDeletion->push($projekt);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collProjekts) {
                foreach ($this->collProjekts as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
